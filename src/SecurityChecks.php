...
final class SecurityChecks {
	...
	private function maybe_add_used_accesskey( $user_identifier = '' ) {
		...
		$used_accesskeys = (array) Utils::get_site_transient( $this->used_accesskey_transient );
		...
		if ( ! in_array( $user_identifier, $used_accesskeys, true ) ) {
			...
			$transient_set = Utils::set_site_transient( $this->used_accesskey_transient, $used_accesskeys, self::ACCESSKEY_LIMIT_EXPIRY );
	...
	private function do_lockdown() {
		...
		$transient_set = Utils::set_site_transient( $this->in_lockdown_transient, time(), self::LOCKDOWN_EXPIRY );
	...
	public function in_lockdown(){
		...
		return Utils::get_site_transient( $this->in_lockdown_transient );
	}
