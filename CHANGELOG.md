...
- Support users whose access has expired are now removed within an hour, even if your plugin was inactive when their access ran out, for example during a plugin update. Previously such an account could outlive its access window. They are revoked the same way as at expiry: the TrustedLogin dashboard is updated and the `revoked` webhook is sent. The check runs on an hourly event WordPress core already schedules, so the SDK adds no cron event of its own. If another plugin removes that event, the check runs on admin page loads instead, at most once an hour.
